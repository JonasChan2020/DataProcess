    ...
    <el-card>
      <div class="list-btn-container">
        <el-button-group>
          ...
          >启用</el-button>
          <el-button
            v-hasPermi="['Ds_classify/DeleteSoft']"
            type="warning"
            icon="el-icon-delete"
            size="small"
            @click="deleteSoft('0')"
    ...
    saveEditForm () {
      this.$refs['editFrom'].validate((valid) => {
        if (valid) {
          ...
          var url = 'Ds_classify/Insert'
          if (this.currentId !== '') {
            url = 'Ds_classify/Update?id=' + this.currentId
          }
          saveDs_classify(data, url).then(res => {
            if (res.Success) {
              this.$message({
                message: '恭喜你，操作成功',
                type: 'success'
