  ...
  <div class="app-container">
    ...
    <el-card>
      <div class="list-btn-container">
        <el-button-group>
          ...
          >修改</el-button>
          <el-button
            v-hasPermi="['Conf_classify/Enable']"
            type="info"
          ...
          >启用</el-button>
          <el-button
            v-hasPermi="['Conf_classify/DeleteSoft']"
            type="warning"
            icon="el-icon-delete"
            size="small"
            @click="deleteSoft('0')"
          >软删除</el-button>
          <el-button
            v-hasPermi="['Conf_classify/Delete']"
            type="danger"
            icon="el-icon-delete"
            size="small"
            @click="deletePhysics()"
      ...
      <el-table ref="gridtable" v-loading="tableloading" :data="tableData" row-key="Id" border stripe highlight-current-row style="width: 100%" default-expand-all :tree-props="{ children: 'Children' }" @select="handleSelectChange" @select-all="handleSelectAllChange" @sort-change="handleSortChange">
        ...
        <el-table-column prop="Ccode" label="类型编码" sortable="custom" width="380" />
        <el-table-column prop="Cname" label="类型名称" sortable="custom" width="180" />
    ...
    >
      ...
      <el-form ref="editFrom" :model="editFrom" :rules="rules">
        <el-form-item label="类型编码" :label-width="formLabelWidth" prop="Ccode">
          <el-input v-model="editFrom.Ccode" placeholder="请输入类型编码" autocomplete="off" clearable />
        </el-form-item>
        <el-form-item label="类型名称" :label-width="formLabelWidth" prop="Cname">
          <el-input v-model="editFrom.Cname" placeholder="请输入类型名称" autocomplete="off" clearable />
        </el-form-item>
...
<script>
  ...
  import {
    getConf_classifyDetail,
    saveConf_classify, setConf_classifyEnable, deleteSoftConf_classify,
    deleteConf_classify, getAllClassifyTreeTable
    ...
    saveEditForm () {
      this.$refs['editFrom'].validate((valid) => {
        if (valid) {
          ...
          saveConf_classify(data, url).then(res => {
            if (res.Success) {
              this.$message({
                message: '恭喜你，操作成功',
                type: 'success'
