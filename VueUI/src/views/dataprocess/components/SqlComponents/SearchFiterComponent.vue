<template>
  <div>
    <el-form ref="editSubFrom" :model="editSubFrom" :rules="Subrules">
      <el-button @click.prevent="addStartDomain()" type="text" v-if="editSubFrom.dynamicFiterForm==null||editSubFrom.dynamicFiterForm.length==0">添加</el-button>
      <el-button @click.prevent="addStartDomainKh()" type="text" v-if="editSubFrom.dynamicFiterForm==null||editSubFrom.dynamicFiterForm.length==0">括号</el-button>
      <el-form-item v-for="(domain, index) in editSubFrom.dynamicFiterForm"
                    :key="index">
        ...
        <el-select v-model="domain.columnName" placeholder="请选择字段" style="width:15%" v-if="(domain.KhType==-1)">
          <el-option v-for="item in SelectColumnNameList"
                     :key="item.FieldName"
                     :label="item.FieldName"
                     :value="item.FieldName" />
        </el-select>
        <el-select v-model="domain.rex" placeholder="请选择操作符" style="width:10%" v-if="(domain.KhType==-1)">
          <el-option v-for="item in SelectRexList"
                     :key="item"
                     :label="item"
                     :value="item" />
        </el-select>
        <el-select v-model="domain.valuetype"
                   placeholder="请选择值类型"
                   style="width:15%"
                   v-if="(domain.KhType==-1)">
          <el-option v-for="item in SelectValueTypeList"
                     :key="item.Key"
                     :label="item.Value"
                     :value="item.Key" />
        </el-select>
        <el-input v-model="domain.value" style="width:20%" v-if="(domain.KhType==-1&&domain.valuetype==2)"></el-input>
        <el-select v-model="domain.tbName"
                   placeholder="请选择表"
                   style="width:15%"
                   v-if="(domain.KhType==-1&&domain.valuetype==1)">
          <el-option v-for="item in tbList"
                     :key="item"
                     :label="item"
                     :value="item" />
        </el-select>
        <el-select v-model="domain.colName"
                   placeholder="请选择字段"
                   style="width:15%"
                   v-if="(domain.KhType==-1&&domain.valuetype==1)">
          <el-option v-for="item in colList"
                     :key="item"
                     :label="item"
                     :value="item" />
        </el-select>
