    ...
    <el-card>
      <div class="list-btn-container">
        <el-button-group>
          ...
          >启用</el-button>
          <el-button
            v-hasPermi="['Plug_type/DeleteSoft']"
            type="warning"
            icon="el-icon-delete"
            size="small"
            @click="deleteSoft('0')"
...
:tree-props="{ children: 'Children' }"
      ...
      >
        ...
        <el-table-column prop="Ptcode" label="类型编码" sortable="custom" width="380" />
        <el-table-column prop="Ptname" label="类型名称" sortable="custom" width="180" />
...
import {
  getPlug_typeDetail,
  savePlug_type, setPlug_typeEnable, deleteSoftPlug_type,
  deletePlug_type, getAllClassifyTreeTable
  ...
  methods: {
    ...
    saveEditForm() {
      this.$refs['editFrom'].validate((valid) => {
        if (valid) {
          ...
          var url = 'Plug_type/Insert'
          if (this.currentId !== '') {
            url = 'Plug_type/Update?id=' + this.currentId
          }
          savePlug_type(data, url).then(res => {
            if (res.Success) {
              this.$message({
                message: '恭喜你，操作成功',
                type: 'success'
    ...
    setEnable: function(val) {
      if (this.currentSelected.length === 0) {
        ...
        setPlug_typeEnable(data).then(res => {
          if (res.Success) {
            this.$message({
              message: '恭喜你，操作成功',
              type: 'success'
    ...
    deleteSoft: function(val) {
      if (this.currentSelected.length === 0) {
        ...
        deleteSoftPlug_type(data).then(res => {
          if (res.Success) {
            this.$message({
              message: '恭喜你，操作成功',
              type: 'success'
